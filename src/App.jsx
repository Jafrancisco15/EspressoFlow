...
// ---------- Utils ----------
function normRect({ x1, y1, x2, y2 }) { const x = Math.min(x1, x2); const y = Math.min(y1, y2); return { x, y, w: Math.abs(x2 - x1), h: Math.abs(y2 - y1) }; }
...
function csvSafe(s) {
  ...
  return `"${q}"`; // comillas CSV correctas
}
...
function giniCoefficient(values) {
  ...
  if (sum === 0) return 0;
  let cum = 0;
  for (let i = 0; i < n; i++) { cum += (2 * (i + 1) - n - 1) * x[i]; }
  return cum / (n * sum);
}
...
function drawChart(canvas, series, spikesIdx, areaSeries) {
  ...
  // extract series
  const xs = series.map((p) => p.t); const ys = series.map((p) => p.jets);
...
function drawMarker(ctx, x, y) { const r = 4; ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fill(); }
