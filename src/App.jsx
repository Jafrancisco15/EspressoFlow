...
function App() {
  ...
  // —— Configuración avanzada (calibración)
  ...
  useEffect(() => {
    (async () => {
      await waitForOpenCV();
      setCvReady(true);
      setStatus('OpenCV listo. Sube tu video y analiza. (ROI y calibración son opcionales)');
    })().catch((err) => setStatus('Error cargando OpenCV: ' + (err?.message || err)));
  }, []);
  ...
  const onVideoFile = async (e) => {
    ...
    setTimeout(async () => {
      try {
        ...
        if (!v) return;
        await ensureReady(v);
        setStatus('Video listo. Se tomará un cuadro automáticamente para permitir ROI.');
        // En mobile, captura automática de un cuadro para habilitar el canvas/ROI
        setTimeout(() => { grabFrame().catch(() => {}); }, 0);
      } catch (err) {
  ...
  // mouse handlers
  const onCanvasMouseDown = (e) => { const p = getPoint(e, e.target); onCanvasPointerDown(p.x, p.y); };
  const onCanvasMouseMove = (e) => { const p = getPoint(e, e.target); onCanvasPointerMove(p.x, p.y); };
  const onCanvasMouseUp   = (e) => { const p = getPoint(e, e.target); onCanvasPointerUp(p.x, p.y); };

  // touch handlers (mobile)
  const onCanvasTouchStart = (e) => { e.preventDefault(); const p = getPoint(e, e.target); onCanvasPointerDown(p.x, p.y); };
  const onCanvasTouchMove  = (e) => { e.preventDefault(); const p = getPoint(e, e.target); onCanvasPointerMove(p.x, p.y); };
  const onCanvasTouchEnd   = (e) => { e.preventDefault(); const p = getPoint(e, e.target); onCanvasPointerUp(p.x, p.y); };
  ...
  useEffect(() => {
    ...
    attachChartClick(chartRef.current, metrics.series, async (tRel) => {
      ...
      const detail = await computeFlowDetailAt(videoRef.current, roi, scale, metrics.avgFlow_gps, minBlobArea);
      ...
      renderOverlayAtCurrent(videoRef.current, roi, scale, minBlobArea, canvasRef.current);
    });
  }, [metrics]);
  ...
  const saveExperiment = () => {
    ...
    setHistory(next);
    try { localStorage.setItem('espressoHistory', JSON.stringify(next)); } catch (e) {}
  };
  ...
  const clearHistory = () => {
    setHistory([]);
    try { localStorage.removeItem('espressoHistory'); } catch (e) {}
  };
  ...
  const exportHistoryCSV = () => {
    ...
    a.href = url; a.download = 'espresso_history.csv';
          ...
          <>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <canvas
                ...
                width={900}
                height={200}
                style={{ width: '100%', border: '1px solid #232339', borderRadius: '8px', cursor: 'pointer' }}
                title="Haz clic para saltar al frame"
              />
            ...
            {inspect && (
              <div className="card" style={{ marginTop: 12 }}>
                ...
                <p className="small muted">
                  Chorros detectados: {inspect.jets ?? (inspect.flows ? inspect.flows.length : '—')}
                  {' · '}Flujo total estimado: {metrics.avgFlow_gps != null ? `${metrics.avgFlow_gps.toFixed(3)} g/s` : '—'}
                </p>
                {inspect.flows ? (
                  <ol className="small">
                ...
                <p className="small muted">
                  Gini: {inspect.gini != null ? inspect.gini.toFixed(3) : '—'} · Máx cuota: {inspect.maxShare != null ? (inspect.maxShare * 100).toFixed(1) + '%' : '—'}
                </p>
              </div>
            )}
...
function roiRectScaled(roi, s) {
  return { x: Math.round(roi.x * s), y: Math.round(roi.y * s), w: Math.round(roi.w * s), h: Math.round(roi.h * s) };
}
function drawRect(ctx, r, color = '#60a5fa') {
  ctx.save();
...
function drawMaskContours(ctx, maskMat, offsetX, offsetY, color = '#60a5fa', minArea = 40) {
  const cv = window.cv;
...
function median(arr) { if (!arr || !arr.length) return null; const a = [...arr].sort((x, y) => x - y); const n = a.length; return n % 2 ? a[(n - 1) / 2] : (a[n / 2 - 1] + a[n / 2]) / 2; }
function clamp(x, lo, hi) { return Math.max(lo, Math.min(hi, x)); }
function mapRange(x, inMin, inMax, outMin, outMax) { if (inMax === inMin) return outMin; const t = (x - inMin) / (inMax - inMin); return outMin + clamp(t, 0, 1) * (outMax - outMin); }
...
async function computeFlowDetailAt(video, roi, scale, avgFlow_gps, minArea = 35) {
  const W = Math.round(video.videoWidth * scale);
  const H = Math.round(video.videoHeight * scale);
  ...
  const areas = componentAreasCC(m, minArea);
  const jets = areas.length;
  let flows = null; let gini = null; let maxShare = null;
...
async function renderOverlayAtCurrent(video, roi, scale, minArea = 40, canvasEl) {
  if (!canvasEl) return;
  const W = Math.round(video.videoWidth * scale);
  const H = Math.round(video.videoHeight * scale);
  const tmp = document.createElement('canvas');
  ...
  octx.drawImage(tmp, 0, 0);
  if (roi) drawRect(octx, r, '#60a5fa');
  drawMaskContours(octx, m, r.x, r.y, '#60a5fa', minArea);
...
function drawChart(canvas, series, spikesIdx, areaSeries) {
  ...
  const ys = series.map((p) => p.jets);
  if (xs.length < 2) return;
  const x0 = xs[0]; const x1 = xs[xs.length - 1];
  ...
  // axes labels (minimal)
  ctx.fillStyle = '#a1a1aa'; ctx.font = '12px ui-sans-serif';
...
function attachChartClick(canvas, series, onJump) {
  ...
  canvas.ontouchstart = (e) => { handler(e); e.preventDefault(); };
}

function generateRecommendations(m) {
  const R = [];
  if (m.jets_cv > 0.45 || m.spikeRate > 0.2 || m.areaJumpRate > 0.15) {
    R.push('Canalización evidente: mejora distribución (WDT/rasp), tamper uniforme y revisa nivelado.');
  }
  if (m.series.length >= 5 && m.series.slice(0, 5).some(fr => fr.jets >= 6)) {
    R.push('Exceso de chorros al inicio: prueba molienda más fina o reducción de ratio para estabilizar la malla.');
  }
  if (m.maxShareMed != null && m.maxShareMed > 0.45) {
    R.push('Un chorro domina el caudal: revisa distribución, posible grumo o hueco en canasta.');
  }
  if (m.avgFlow_gps != null && m.avgFlow_gps > 2.5) {
    R.push('Flujo alto para espresso clásico: considera más fino / mayor dosis o canasta más restrictiva.');
  }
  if (m.avgFlow_gps != null && m.avgFlow_gps < 1.2) {
    R.push('Flujo bajo: prueba un poco más grueso o sube temperatura/presión según tu máquina.');
  }
  if (m.duration < 18) R.push('Extracción muy corta: aumenta tiempo/afinamiento para extraer más parejo.');
  if (m.duration > 40) R.push('Extracción muy larga: evalúa molienda más gruesa o ratio mayor.');
  if (!R.length) R.push('Extracción estable. Afina por sabor: ajusta 0.1–0.2 bar o 0.1–0.2 g de dosis.');
  return R;
}
