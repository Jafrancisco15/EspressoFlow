...
// —— Configuración avanzada (calibración) const [showConfig, setShowConfig] = useState(false); const [spikeDelta, setSpikeDelta] = useState(2); const [spikeMinJets, setSpikeMinJets] = useState(3); const [minBlobArea, setMinBlobArea] = useState(35); const [areaJumpFactor, setAreaJumpFactor] = useState(1.35);
...
useEffect(() => { (async () => { await waitForOpenCV(); setCvReady(true); setStatus('OpenCV listo. Sube tu video y analiza. (ROI y calibración son opcionales)'); })().catch((err) => setStatus('Error cargando OpenCV: ' + (err?.message || err))); }, []);

useEffect(() => { try { const raw = localStorage.getItem('espressoHistory'); if (raw) setHistory(JSON.parse(raw)); } catch {} }, []);

const onVideoFile = async (e) => { const file = e.target.files?.[0]; if (!file) return; const url = URL.createObjectURL(file); setVideoURL(url); setFrameLoaded(false); setRoi(null); setMetrics(null); setInspect(null); setRecs([]); setStatus('Cargando video...'); setTimeout(async () => { try { const v = videoRef.current; if (!v) return; await ensureReady(v); setStatus("Video listo. Se tomará un cuadro automáticamente para permitir ROI."); // En mobile, captura automática de un cuadro para habilitar el canvas/ROI setTimeout(() => { grabFrame().catch(()=>{}); }, 0); } catch (err) { setStatus('No se pudo preparar el video: ' + (err?.message || err)); } }, 50); };
...
// ---- ROI drawing (mouse + touch) ---- const getPoint = (evt, target) => { const rect = target.getBoundingClientRect(); const touch = evt.touches?.[0] || evt.changedTouches?.[0]; const cx = touch ? touch.clientX : evt.clientX; const cy = touch ? touch.clientY : evt.clientY; return { x: cx - rect.left, y: cy - rect.top }; };

const onCanvasPointerDown = (x, y) => { if (!frameLoaded) { grabFrame(); return; } drawState.current = { dragging: true, start: { x, y } }; }; const onCanvasPointerMove = (x, y) => { if (!drawState.current.dragging) return; const ctx = canvasRef.current.getContext('2d'); ctx.drawImage(videoRef.current, 0, 0, canvasRef.current.width, canvasRef.current.height); if (roi) drawRect(ctx, roi, '#60a5fa'); const s = drawState.current.start; const r = normRect({ x1: s.x, y1: s.y, x2: x, y2: y }); drawRect(ctx, r, '#60a5fa'); }; const onCanvasPointerUp = (x, y) => { if (!drawState.current.dragging) return; drawState.current.dragging = false; const s = drawState.current.start; const r = normRect({ x1: s.x, y1: s.y, x2: x, y2: y }); if (r.w >= 10 && r.h >= 10) setRoi(r); };

// mouse handlers const onCanvasMouseDown = (e) => { const p = getPoint(e, e.target); onCanvasPointerDown(p.x, p.y); }; const onCanvasMouseMove = (e) => { const p = getPoint(e, e.target); onCanvasPointerMove(p.x, p.y); }; const onCanvasMouseUp   = (e) => { const p = getPoint(e, e.target); onCanvasPointerUp(p.x, p.y); };

// touch handlers (mobile) const onCanvasTouchStart = (e) => { e.preventDefault(); const p = getPoint(e, e.target); onCanvasPointerDown(p.x, p.y); }; const onCanvasTouchMove  = (e) => { e.preventDefault(); const p = getPoint(e, e.target); onCanvasPointerMove(p.x, p.y); }; const onCanvasTouchEnd   = (e) => { e.preventDefault(); const p = getPoint(e, e.target); onCanvasPointerUp(p.x, p.y); };

useEffect(() => { if (frameLoaded) drawOverlay(); }, [roi, frameLoaded]);

const drawOverlay = () => { const canvas = canvasRef.current; const video = videoRef.current; if (!canvas || !video) return; const ctx = canvas.getContext('2d'); ctx.drawImage(video, 0, 0, canvas.width, canvas.height); if (roi) drawRect(ctx, roi, '#60a5fa'); };
...
let startIdx = 0; let endIdx = perFrameFull.length - 1;
...
let giniMed = null; let maxShareMed = null;
if (avgFlow_gps != null) {
  const ginis = []; const sharesMax = [];
...
// ---- DIBUJO DEL GRÁFICO ---- useEffect(() => { if (!metrics) return; const areaSeries = metrics.series.map((p) => p.area); drawChart(chartRef.current, metrics.series, metrics.spikesIdx, areaSeries); attachChartClick(chartRef.current, metrics.series, async (tRel) => { const tAbs = metrics.startSec + tRel; await seekTo(videoRef.current, tAbs); const detail = await computeFlowDetailAt(videoRef.current, roi, scale, metrics.avgFlow_gps, minBlobArea); const idx = findNearestIndex(metrics.series, tRel); if (idx >= 0) { const fr = metrics.series[idx]; if (metrics.avgFlow_gps != null && fr.areas?.length) { const sumA = fr.areas.reduce((a, b) => a + b, 0); const flows = sumA > 0 ? fr.areas.map((a) => (a / sumA) * metrics.avgFlow_gps) : null; const gini = flows ? giniCoefficient(flows) : null; const maxShare = sumA > 0 ? Math.max(...fr.areas.map((a) => a / sumA)) : null; setInspect({ tAbs, tRel, jets: fr.jets, flows, gini, maxShare }); } else { setInspect({ tAbs, tRel, jets: detail.jets, flows: detail.flows, gini: detail.gini, maxShare: detail.maxShare }); } } else { setInspect(detail); } renderOverlayAtCurrent(videoRef.current, roi, scale, minBlobArea, canvasRef.current); }); }, [metrics]);

// ------- Guardar experimento (historial) ------- const saveExperiment = () => { if (!metrics) return; const exp = { id: Date.now(), timestamp: new Date().toISOString(), dose: parseFloatOrNull(dose), output: parseFloatOrNull(output), tds: parseFloatOrNull(tds), ey: EY !== null ? round2(EY) : null, balance: Number(balance) || null, notes: (notes || '').trim() || null, score: metrics.score, jets_mean: round2(metrics.jets_mean), jets_cv: round2(metrics.jets_cv), spikeRate: round2(metrics.spikeRate * 100), areaJumpRate: round2(metrics.areaJumpRate * 100), duration: round2(metrics.duration), frames: metrics.frames, avgFlow_gps: metrics.avgFlow_gps !== null ? round3(metrics.avgFlow_gps) : null, giniMed: metrics.giniMed !== null ? round3(metrics.giniMed) : null, maxShareMed: metrics.maxShareMed !== null ? round3(metrics.maxShareMed) : null, }; const next = [exp, ...history]; setHistory(next); try { localStorage.setItem('espressoHistory', JSON.stringify(next)); } catch {} };

const deleteExperiment = (id) => { const next = history.filter((x) => x.id !== id); setHistory(next); try { localStorage.setItem('espressoHistory', JSON.stringify(next)); } catch {} }; const clearHistory = () => { setHistory([]); try { localStorage.removeItem('espressoHistory'); } catch {} };

const exportHistoryCSV = () => { const header = 'timestamp,dose_g,output_g,tds_pct,ey_pct,balance,notes,score,jets_mean,jets_cv,spike_rate_pct,area_jump_rate_pct,duration_s,frames,avg_flow_gps,gini_med,max_share_med '; const rows = history.map((h) => [ h.timestamp, val(h.dose), val(h.output), val(h.tds), val(h.ey), val(h.balance), csvSafe(h.notes), h.score, h.jets_mean, h.jets_cv, h.spikeRate, h.areaJumpRate, h.duration, h.frames, val(h.avgFlow_gps), val(h.giniMed), val(h.maxShareMed), ].join(',')).join(' '); const blob = new Blob([header + rows], { type: 'text/csv' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'espresso_history.csv'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url); };

const exportShotCSV = () => { if (!metrics) return; const header = 't_sec,jets,area,spike,areaJump '; const rows = metrics.series.map((p, i) => [ p.t.toFixed(3), p.jets, p.area, metrics.spikesIdx.includes(i) ? 1 : 0, metrics.areaJumpIdx.includes(i) ? 1 : 0, ].join(',')).join(' '); const blob = new Blob([header + rows], { type: 'text/csv' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'espresso_flow_metrics.csv'; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url); };
...
<div className="kpi" style={{ margin: '12px 0 18px' }}>
  ...
  <div className="card" style={{ marginBottom: 16 }}>
    ...
    {videoURL && (
      <video
        ...
        onLoadedMetadata={() => { setStatus('Video cargado. Preparando vista previa…'); setTimeout(() => { grabFrame().catch(()=>{}); }, 0); }}
      />
    )}
  </div>

  <div className="card" style={{ marginBottom: 16 }}>
    <h3>2) (Opcional) Define ROI y escala</h3>
    ...
    {metrics && (
      <>
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <p className="small muted">Toca o haz clic en el gráfico para saltar al frame y ver detalle de flujos estimados.</p>
        </div>
  ...
  <div className="card" style={{ marginBottom: 16 }}>
    <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
      <h3 style={{ margin: 0 }}>Configuración (opcional)</h3>
      <button className="btn secondary" onClick={() => setShowConfig(!showConfig)}>{showConfig ? 'Ocultar' : 'Mostrar'}</button>
    </div>
    {showConfig && (
...
// extract series const xs = series.map((p) => p.t); const ys = series.map((p) => p.jets); if (xs.length < 2) return; const x0 = xs[0]; const x1 = xs[xs.length - 1]; const yMax = Math.max(2, ...ys);
...
// axes labels (minimal) ctx.fillStyle = '#a1a1aa'; ctx.font = '12px ui-sans-serif'; ctx.fillText('0s', 2, H - 4); ctx.fillText((x1 - x0).toFixed(1) + 's', W - 40, H - 4); } function drawMarker(ctx, x, y) { const r = 4; ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fill(); } function attachChartClick(canvas, series, onJump) { if (!canvas || !series?.length) return; const xs = series.map((p) => p.t); const x0 = xs[0]; const x1 = xs[xs.length - 1]; const handler = (e) => { const rect = canvas.getBoundingClientRect(); const x = (e.clientX ?? (e.touches?.[0]?.clientX || 0)) - rect.left; const t = (x / rect.width) * (x1 - x0) + x0; if (onJump) onJump(t); }; canvas.onclick = handler; canvas.ontouchstart = (e) => { handler(e); e.preventDefault(); }; }

function generateRecommendations(m) { const R = []; if (m.jets_cv > 0.45 || m.spikeRate > 0.2 || m.areaJumpRate > 0.15) { R.push('Canalización evidente: mejora distribución (WDT/rasp), tamper uniforme y revisa nivelado.'); } if (m.series.length >= 5 && m.series.slice(0, 5).some(fr => fr.jets >= 6)) { R.push('Exceso de chorros al inicio: prueba molienda más fina o reducción de ratio para estabilizar la malla.'); } if (m.maxShareMed != null && m.maxShareMed > 0.45) { R.push('Un chorro domina el caudal: revisa distribución, posible grumo o hueco en canasta.'); } if (m.avgFlow_gps != null && m.avgFlow_gps > 2.5) { R.push('Flujo alto para espresso clásico: considera más fino / mayor dosis o canasta más restrictiva.'); } if (m.avgFlow_gps != null && m.avgFlow_gps < 1.2) { R.push('Flujo bajo: prueba un poco más grueso o sube temperatura/presión según tu máquina.'); } if (m.duration < 18) R.push('Extracción muy corta: aumenta tiempo/afinamiento para extraer más parejo.'); if (m.duration > 40) R.push('Extracción muy larga: evalúa molienda más gruesa o ratio mayor.'); if (!R.length) R.push('Extracción estable. Afina por sabor: ajusta 0.1–0.2 bar o 0.1–0.2 g de dosis.'); return R; }
