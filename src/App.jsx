...
function App() {
  ...
  useEffect(() => {
    ...
    attachChartClick(chartRef.current, metrics.series, async (tRel) => {
      ...
      const detail = await computeFlowDetailAt(
        videoRef.current, roi, scale, metrics.avgFlow_gps, minBlobArea
      );
      ...
      if (idx >= 0) {
        ...
        if (metrics.avgFlow_gps != null && fr.areas?.length) {
          ...
          setInspect({
            tAbs, tRel,
            jets: detail.jets, flows: detail.flows, gini: detail.gini, maxShare: detail.maxShare
          });
        }
      } else {
        setInspect(detail);
      }
      // 🔧 pasa el canvas explícitamente
      await renderOverlayAtCurrent(videoRef.current, roi, scale, minBlobArea, canvasRef.current);
    });
  }, [metrics]);
  ...
  const exportHistoryCSV = () => {
    const header =
      'timestamp,dose_g,output_g,tds_pct,ey_pct,balance,notes,score,jets_mean,jets_cv,spike_rate_pct,area_jump_rate_pct,duration_s,frames,avg_flow_gps,gini_med,max_share_med\n';
...
// ---- Heurísticas de recomendaciones ----
function generateRecommendations(m) {
  ...
  // Un chorro dominante (si hay flujo estimado)
