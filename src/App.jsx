...
frame.delete(); A.delete(); g.delete(); m.delete(); kernel.delete(); }; let A = frame.roi(new cv.Rect(r.x, r.y, r.w, r.h)); let g = new cv.Mat(); cv.cvtColor(A, g, cv.COLOR_RGBA2GRAY); cv.GaussianBlur(g, g, new cv.Size(3, 3), 0); let m = new cv.Mat(); cv.adaptiveThreshold(g, m, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 21, 5); let kernel = cv.getStructuringElement(cv.MORPH_ELLIPSE, new cv.Size(3, 3)); cv.morphologyEx(m, m, cv.MORPH_OPEN, kernel);
...
// axes labels (minimal) ctx.fillStyle = '#a1a1aa'; ctx.font = '12px ui-sans-serif'; ctx.fillText('0s', 2, H - 4); ctx.fillText((x1 - x0).toFixed(1) + 's', W - 40, H - 4); } function drawMarker(ctx, x, y) { const r = 4; ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fill(); } function attachChartClick(canvas, series, onJump) { if (!canvas || !series?.length) return; const xs = series.map((p) => p.t); const x0 = xs[0]; const x1 = xs[xs.length - 1]; const handler = (e) => { const rect = canvas.getBoundingClientRect(); const x = e.clientX - rect.left; const t = (x / rect.width) * (x1 - x0) + x0; if (onJump) onJump(t); }; canvas.onclick = handler; }
